...
class Storage_Model_DbTable_Files extends Engine_Db_Table
{
    ...
    public function getMultiFiles($ids) {
        if (!empty($ids)) {
            ...
            foreach ($files as $file) {
                $key = $file->parent_file_id . '_' . $file->type;
                if (!array_key_exists($key, $this->_files)) {
                    $this->_files[$key] = $file;
                }
            }
    ...
    public function getFiles($ids, $relationship = null)
    {
        //$key = $id . '_' . ( $relationship ? $relationship : 'default' );
        $keys = array();
        $unexistedKeys = array();
        ...
        foreach ($ids as $id) {
            $key = $id . '_' . ( $relationship ? $relationship : 'default' );
            if (!array_key_exists($key, $this->_files)) {
                array_push($unexistedKeys, $key);
                array_push($unexistedIds, $id);
            }
        }
        if (!empty($unexistedKeys)) {
            $files = array();
            if ($relationship) {
                $select = $this->select()->where('parent_file_id IN (?)', $unexistedIds)->where('type = ?', $relationship);
                
                $files = $this->fetchAll($select);
            }
            
            if (empty($files)) {
                $files = Engine_Api::_()->getItemMulti('storage_file', $unexistedIds);
            }
            
            foreach ($files as $file) {
                $key = $file->getIdentity() . '_' . ( $relationship ? $relationship : 'default' );
                $this->_files[$key] = $file;
            }
        }
        ...
        foreach ($this->_files as $file) {
            if (in_array($file->getIdentity(), $ids)) {
                array_push($returnedFiles, $file);
            }
        }
