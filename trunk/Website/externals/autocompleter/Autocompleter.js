...
var Autocompleter = new Class({
	...
	choiceSelect : function(choice) {
		...
		if (this.options.autocompleteType == 'message') {
			...
			if (token.friends) {
				...
				this.doAddValueToHidden(token.label, friend_ids, this.options.hiddenElementId, true, ' tag_friend');
				...
				this.doAddValueToHidden(choice.id, token.id, this.options.hiddenElementId, true);
		}
	...
	doPushSpan : function(name, toID, newItem, hideLoc, list) {
		...
		$(hideLoc + '-wrapper').setStyle('height', 'auto');

		if (list == null)
			list = "";
		myElement.addClass("tag" + list);

		document.getElementById(hideLoc + '-element').appendChild(myElement);
		this.fireEvent('push');
	},
