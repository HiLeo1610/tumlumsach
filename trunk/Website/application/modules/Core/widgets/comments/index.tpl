...
<?php if( !$this->page ): ?>
<div class='comments' id="comments-<?php echo $this->subject()->getGuid()?>">
<?php endif; ?>
  <div class='comments_options'>
    ...
    <?php if( isset($this->form) ): ?>
      - <a href='javascript:void(0);' onclick="$('comment-form-<?php echo $this->subject()->getGuid()?>').style.display = '';$('comment-form-<?php echo $this->subject()->getGuid()?>').body.focus();"><?php echo $this->translate('Post Comment') ?></a>
    <?php endif; ?>

    <?php if( $this->viewer()->getIdentity() && $this->canComment ): ?>
      <?php if( $this->subject()->likes()->isLike($this->viewer()) ): ?>
        - <a href="javascript:void(0);" onclick="en4.core.comments.unlike('<?php echo $this->subject()->getType()?>', '<?php echo $this->subject()->getIdentity() ?>', '', '<?php echo $this->subject()->getGuid()?>')"><?php echo $this->translate('Unlike This') ?></a>
      <?php else: ?>
        - <a href="javascript:void(0);" onclick="en4.core.comments.like('<?php echo $this->subject()->getType()?>', '<?php echo $this->subject()->getIdentity() ?>', '', '<?php echo $this->subject()->getGuid()?>')"><?php echo $this->translate('Like This') ?></a>
      <?php endif; ?>
    <?php endif; ?>
  </div>
  <ul>
              ...
              <?php if( $this->canComment ):
                ...
                <?php if( !$isLiked ): ?>
                  <a href="javascript:void(0)" 
                  	onclick="en4.core.comments.like(<?php echo sprintf("'%s', %d, %d, '%s'", $this->subject()->getType(), $this->subject()->getIdentity(), $comment->getIdentity(), $this->subject()->getGuid()) ?>)">
                    <?php echo $this->translate('like') ?>
                  </a>
                <?php else: ?>
                  <a href="javascript:void(0)" onclick="en4.core.comments.unlike(<?php echo sprintf("'%s', %d, %d, '%s'", $this->subject()->getType(), $this->subject()->getIdentity(), $comment->getIdentity(), $this->subject()->getGuid()) ?>)">
  ...
  <script type="text/javascript">
    en4.core.runonce.add(function(){
      $($('comment-form-<?php echo $this->subject()->getGuid()?>').body).autogrow();
      en4.core.comments.attachCreateComment($('comment-form-<?php echo $this->subject()->getGuid()?>'), '<?php echo $this->subject()->getGuid()?>');
    });
  </script>
  <?php if( isset($this->form) ) echo $this->form->setAttribs(array('id' => 'comment-form-' . $this->subject()->getGuid(), 'style' => 'display:none;'))->render() ?>
<?php if( !$this->page ): ?>
</div>
<?php endif; ?>